...
# SBOM inspection.
RUN mkdir -p /mnt/rootfs && dnf update -y --nodocs --setopt=keepcache=0 --refresh && \
 dnf install --installroot /mnt/rootfs -y --nodocs --setopt=keepcache=0 --setopt \
 install_weak_deps=false jq gzip ca-certificates && dnf --installroot /mnt/rootfs clean all
